...
if __name__ == "__main__":
    ...
    for folder in range(len(folders)):
        ...
        S_temporal = pd.read_csv(f"../../step2_graphRepresentation/{way_to_build_graph}/s{str(int(folders[folder])+1)}/graph_Xtr_th_0.975.csv")
        S_static = pd.read_csv(f"../../step2_graphRepresentation/{way_to_build_graph}/s{str(int(folders[folder])+1)}/static_graph_Xtr_th_0.975.csv")
